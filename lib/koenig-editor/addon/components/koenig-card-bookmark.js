...
export default Component.extend({
    ...
    counts: computed('payload.{metadata,caption}', function () {
        let imgCount = 0;
        let wordCount = 0;
        imgCount = this.payload.metadata.icon ? (imgCount + 1) : imgCount;
        imgCount = this.payload.metadata.thumbnail ? (imgCount + 1) : imgCount;
        wordCount = countWords(this.payload.caption) + countWords(this.payload.metadata.title) + countWords(this.payload.metadata.description);
        return {
            imageCount: imgCount,
            wordCount: wordCount
        };
