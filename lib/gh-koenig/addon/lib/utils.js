...
export function checkIfClickEventShouldCloseCard(target, cardHolder) {
    ...
    let card = target.hasClass('js-kg-card') ? target : target.parents('.js-kg-card');

    let isCardToggle = target.hasClass('kg-card-button') || target.parents('.js-gh-cardmenu').length || target.parents('.js-kg-card-toolbar').length;
...
// PR to mobiledoc incoming to put in official remove event listener functionality.
export function deleteListener(editor, queue, func) {
    let cbQueue = editor._callbacks.callbackQueues[queue];
    for (let i = cbQueue.length - 1; i > -1; i--) {
        if (cbQueue[i] === func) {
            cbQueue.splice(i, 1);
            return;
        }
