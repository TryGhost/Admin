...
export default Component.extend({
    ...
    willRender() {
        ...
        editor.willRender(() => {
            if (!run.currentRunLoop) {
                run.begin();
                this.set('inRunLoop', true);
            }
        });

        editor.didRender(() => {
            if (this.get('inRunLoop')) {
                run.end();
                this.set('inRunLoop', false);
            }
    ...
    didRender() {
        ...
        let editor = this.get('editor');
        let editorHolder = this.$('.js-gh-koenig');
        let [domContainer] = editorHolder.parents(this.get('containerSelector'));
        let [editorDom] = editorHolder;
    ...
    cursorMoved() {
        ...
        if (editor.range.isCollapsed) {
            ...
            if (editor.range && editor.range.headSection && editor.range.headSection.isCardSection) {
                let id = $(editor.range.headSection.renderNode.element).find('.js-kg-card > div').attr('id');
    ...
    actions: {
        ...
        selectCard(cardId) {
            ...
            let cardHolder = $(`#${cardId}`).parent('.js-kg-card');
        ...
        selectCardHard(cardId) {
            ...
            let card = this.get('emberCards').find((card) => card.id === cardId);
            let cardHolder = $(`#${cardId}`).parents('.js-kg-card');
            let selectedCard = this.get('selectedCard');
            if (selectedCard && selectedCard !== card) {
                this.send('deselectCard');
            }
        ...
        deselectCard() {
            let selectedCard = this.get('selectedCard');
            if (selectedCard) {
                let cardHolder = $(`#${selectedCard.id}`).parent('.js-kg-card');
