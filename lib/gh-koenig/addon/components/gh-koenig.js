...
export default Component.extend({
    ...
    actions: {
        ...
        selectCard(cardId) {
            console.log("SELECTING CARD2", cardId);
            ...
            if (selectedCard && selectedCard !== card) {
                console.log("10", this.get('selectedCard'));
                this.send('deselectCard');
            }
            if(!cardHolder[0]) {
...
function checkIfClickEventShouldCloseCard(target, cardHolder) {
    ...
    let parentCard = target.parents('.kg-card');
    let isCardToggle = (target.parents('.gh-cardmenu').length || target.parents('.kg-card-toolbar').length);

    if (!isCardToggle && (!parentCard.length || (parentCard.length && parentCard[0] !== cardHolder[0]))) {
        return true;
    }
    return false;
}
