...
export default Component.extend({
    ...
    didRender() {
        ...
        let editor = this.get('editor');
        this.set('domContainer', $(editorDom).parents(this.get('containerSelector'))[0]);
    ...
    // process dirty editor
    processDirty() {
        run.join(() => {
            // store a cache of the local doc so that we don't need to reinitialise it.
            this._cachedDoc = this.get('editor').serialize(MOBILEDOC_VERSION);
            ...
            this.set('wordCount', this.$('.surface')[0].innerText.split(/\W/).length);
        });
    },
    actions: {
