...
export default Component.extend({
    ...
    // Counting of words can get expensive so we throttle it so that it will only update ever 200ms.
    ...
    processWordCount() {
        if (!this.get('isWordCounting')) {
            run.later(() => {
                ...
                this.set('isWordCounting', false);
            }, 200);
            this.set('isWordCounting', true);
        }
    },
    actions: {
