...
export default Component.extend({
    ...
    didRender() {
        ...
        let input = this.$('.js-gh-cardmenu-search-input');
        let editorHolder = $(this.get('containerSelector'));
        ...
        editor.cursorDidChange(() => {
            run(() => {
                ...
                let editorOffset = editorHolder.offset();
                ...
                run.schedule('afterRender', this,
                    () => {
                        let button = this.$('.js-gh-cardmenu-button');
                        button.css('top', offset.top + editorHolder.scrollTop() - editorOffset.top - 2);
                    });
            });
        });
    },
    actions: {
        openMenu: function () { // eslint-disable-line
            let button = this.$('.js-gh-cardmenu-button'); // the ⊕ button.
            let editorHolder = $(this.get('containerSelector'));
            ...
            run.schedule('afterRender', this,
                () => {
                    let menu = this.$('.js-gh-cardmenu');
                    let top = parseInt(button.css('top').replace('px', ''));

                    // calculate the parts of the menu that are hidden by the overflow.
                    let hiddenByOverflow = (editorHolder.innerHeight() + editorHolder.scrollTop()) - (menu.height() + top);
                    ...
                    menu.hide().fadeIn('fast', () => {
                        this.$('.js-gh-cardmenu-search-input').focus();
                    });
                });
        ...
        closeMenu: function () { // eslint-disable-line
            ...
            this.$('.js-gh-cardmenu').fadeOut('fast', () => {
