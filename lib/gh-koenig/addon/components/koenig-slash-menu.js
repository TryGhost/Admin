...
import {getPositionFromRange, deleteListener} from '../lib/utils';
...
export default Component.extend({
    ...
    willDestroy() {
        deleteListener(this.get('editor'), 'cursorDidChange', this.get('cursorChangeCB'));
    },
    ...
    didRender() {
        ...
        let cursorFunc = this.cursorChange.bind(this);
        editor.cursorDidChange(cursorFunc);

        this.set('cursorChangeCB', cursorFunc);
    ...
    cursorChange() {
        ...
        if (!range || !editor.range.isCollapsed || editor.range.head.section !== range.section || this.editor.range.head.offset < 1 || !this.editor.range.head.section) {
            ...
            if (isOpen && !$(window.getSelection().anchorNode).parents('.js-gh-cardmenu').length) {
                this.send('closeMenu');
            }
            return;
        }
